...
class ScopedVASurfaceID {
 public:
  ...
  bool CreateSurface(int format, VASurfaceAttribExternalBuffers& external) {
    ...
    VAStatus ret = vaCreateSurfaces(display_,
                            format,
                            external.width,
                            external.height,
                            &surface_,
...
class ScopedVAConfigID {
 public:
   ...
   bool CreateConfig(int format) {
     ...
     VAStatus ret = vaCreateConfig(display_,
                                   VAProfileNone,
...
class ScopedVAContextID {
 public:
   ...
   bool CreateContext(VAConfigID config, int width, int height,
     ...
     VAStatus ret = vaCreateContext(display_,
                                  config,
                                  width,
                                  height,
                                  0x00,
...
bool VARenderer::Draw(OverlayLayer *layer, NativeSurface *surface) {
  ...
  if (!va_context.CreateContext(va_config.GetConfig(), dest_width,
                        dest_height, surface_out.GetSurface())) {
    return false;
  }
  ...
  param.surface = surface_in.GetSurface();
  ...
  VAStatus ret = VA_STATUS_SUCCESS;
  ret = vaBeginPicture(va_display_, va_context.GetContext(),
                       surface_out.GetSurface());
  ret |= vaRenderPicture(va_display_, va_context.GetContext(),
                         &pipeline_buffer, 1);
  ret |= vaEndPicture(va_display_, va_context.GetContext());
  ret |= vaSyncSurface(va_display_, surface_out.GetSurface());
