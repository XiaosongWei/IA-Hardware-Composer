...
class ScopedVABufferID {
 public:
  ...
  bool CreateBuffer(VAContextID context, VABufferType type,
                          uint32_t size, uint32_t num, void* data) {
    VAStatus ret =  vaCreateBuffer(display_,
                                    context,
                                    type,
                                    size,
                                    num,
...
bool VARenderer::Draw(OverlayLayer *layer, NativeSurface *surface) {
  ...
  if (!va_context.CreateContext(va_config, dest_width,
                        dest_height, surface_out)) {
    return false;
  }
  ...
  if (!pipeline_buffer.CreateBuffer(va_context,
